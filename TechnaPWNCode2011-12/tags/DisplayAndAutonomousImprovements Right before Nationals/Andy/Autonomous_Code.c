...
TAutonomous AutonomousArray[1][21];
...
int processAutonomous()
{
  ...
  // Autonomous State Machine
    ...
    //--------------------------
		...
		if (AutonomousRoutine < NumOfAutonomusRoutines)			//An Autonomous Routine
		{
			...
			memcpy(CurrentStep, AutonomousArray[0][AutonomousStep], sizeof(CurrentStep)); //This does
		}
		else				                                				//No Autonomous
		{
		...
		clearLCDLine(0);
		clearLCDLine(1);
		displayLCDString(0, 0, AutonomousArray[0][0].StepName); // Display Routine Name
  ...
  else if (AutonomousState == DOSTEP)
  {
    ...
    {
 			pneuTines = CurrentStep.DrivePower;
    }
	  // Regardless,
